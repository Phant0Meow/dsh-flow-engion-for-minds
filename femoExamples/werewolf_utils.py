...
def process_votes_and_end(votes, alive_players, roles_dict, wolves):
    """计票 -> 放逐 -> 判胜负（屠边制）。
    只统计投给存活玩家的票（空串/场外名字视为弃票）；平票=无人出局。
    不清空票箱：announce_vote 还要念票，次日由 DayPhase.init 清空。
    同步维护 wolves / wolf_room：被放逐的狼剔出夜间名单（防幽灵之刀）。"""
    alive_players = list(alive_players or [])
    ...
    for voter, target in (votes or {}).items():
        ...
        target = _as_name(target)
        if voter and target and target in alive_set:
            clean[voter] = target
